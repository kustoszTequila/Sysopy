		...
		if(strlen(line) >1)
		{
			...
			while (pch != NULL)
			{
				if (strcmp(pch,"=") == 0)
				{
					...
					if (cArray->components[componentIndex ]->commands == NULL)
					{
						perror("błąd alokacji!\n");
						exit(-1);		
					}
					//
						cArray->components[componentIndex ]->commands[commandIndex]->
						arguments[cArray->components[componentIndex ]->
						commands[commandIndex]->size]
						 = calloc(1,strlen(pch)+1);
					if (cArray->components[componentIndex ]->commands[commandIndex]->
						arguments[cArray->components[componentIndex ]->
						commands[commandIndex]->size] == NULL)
					{
						perror("błąd alokacji!\n");
						exit(-1);		
					}
					...
					{	
						...
						{	
							...
							realloc(cArray->components[componentIndex ]->commands[commandIndex]->arguments,
							cArray->components[componentIndex ]->commands[commandIndex]->size+1);
							if ( cArray->components[componentIndex ]->
							commands[commandIndex]->arguments == NULL)
							{
								perror("błąd alokacji\n");
								exit(1);
							}
			...
			{				
				for (int i = 0; i < cArray->size; i++)
				{
					...
					if(strstr(pch,cArray->components[i]->name) != NULL)
					{
						...
						commands = realloc(commands, comSize + cArray->components[i]->size );
						if ((commands) == NULL)
						{
